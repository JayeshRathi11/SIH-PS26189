...
export default function App() {
  const getInitialTab = () => {
    ...
    const validTabs = ['board', 'timeline', 'cases', 'entities', 'anomalies', 'dossiers', 'xai', 'benchmarks', 'audit'];
  ...
  // this component's local state -- load it once we know who's asking. If the
  // fetch fails (offline, backend down) we keep the built-in fallback list above
  ...
  const handleAddCase = (newCase) => {
    ...
    // Persist to the shared case registry so it survives a refresh and shows
    // up for every officer, not just this browser tab.
