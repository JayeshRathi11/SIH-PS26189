...
import { runPipeline, getAuthHeaders, uploadCaseDocuments } from '../api/client';
...
export default function Sidebar({
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const statusRes = await fetch(`/api/pipeline/status/${pollingJobId}`, { headers: getAuthHeaders() });
  ...
  const handleAddSubmit = async (e) => {
    ...
    // A brand new case has no source documents on disk the way the 10 demo
    // domains do -- runPipeline() would filter existing parsed documents down
    // to zero and silently produce an empty case. Uploading the actual FIR /
    // case documents is what runs real extraction against real content.
    if (newCaseFiles.length === 0) {
      setPollingStatus('Add at least one case document (.txt, .docx, or .pdf) to extract from.');
      return;
    }

    setPollingStatus('Uploading & extracting from case documents...');
    try {
      const data = await uploadCaseDocuments(domainId, newCaseFiles);
      failureCountRef.current = 0;
      ...
      if (data.skipped_files && data.skipped_files.length > 0) {
        setPollingStatus('Some files were skipped: ' + data.skipped_files.join('; '));
      }
    } catch (err) {
      console.error(err);
        ...
        <div className="add-case-block">
          {!showAddForm ? (
            ...
            <form className="add-case-form" onSubmit={handleAddSubmit}>
              <div className="add-case-form-title">New Investigation Case</div>
              <input
              ...
              <input
                ...
                required
              />
              <label className="add-case-dropzone" htmlFor="add-case-file-input">
                <span className="add-case-dropzone-icon">&#128196;&#8593;</span>
                <span className="add-case-dropzone-text">
                  <strong>{newCaseFiles.length > 0 ? 'Change uploaded documents' : 'Upload case documents to extract from'}</strong>
                  <span>.txt, .docx or .pdf &mdash; click to browse</span>
                </span>
              </label>
              <input
                id="add-case-file-input"
                type="file"
                multiple
                accept=".txt,.docx,.pdf"
              ...
              {newCaseFiles.length > 0 && (
                <div className="add-case-file-list">
                  {newCaseFiles.map((f, idx) => (
                    <span key={idx} className="add-case-file-chip">&#128206; {f.name}</span>
                  ))}
                </div>
              )}
              <div className="add-case-form-actions">
                <button type="submit" className="add-case-submit-btn" disabled={!!pollingStatus}>
                  {pollingStatus ? 'Processing…' : 'Create Case'}
                </button>
                <button
                  type="button"
                  className="add-case-cancel-btn"
                  onClick={() => { setShowAddForm(false); setPollingStatus(''); setNewCaseFiles([]); }}
                  disabled={!!pollingStatus}
                >
                  Cancel
                </button>
